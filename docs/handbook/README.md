...
> ## Read this first — status, 2026-09-22
>
> **This handbook was written on 2026-07-11 and has not been re-verified against the
> code since.** It was landed in September, unedited apart from this note and the
> per-page banners, because [#158](https://github.com/otungKe/wepl/issues/158) is named
> after it and the repository did not contain it. Its reasoning is worth having; its
> present-tense claims are two and a half months old.
>
> Known to have changed since it was written:
>
> - **It predates ADR-0028 through ADR-0032.** In particular ADR-0030 decomposes
>   `FinancialTransaction`, which this handbook still describes as a settled part of the
>   model. That decomposition is half-run: the read projection, settlement registry and
>   back-office readers have landed; the rail columns, the public `WEPL-TXN` handle and
>   the model itself have not.
> - **The "reference baseline" claim below is not true as written.** The record and the
>   code had drifted apart by September — the ADR index had lost four entries, two ADR
>   status lines were wrong, and the roadmap still described the pre-Phase-0 world. Those
>   were fixed in #205, #206 and #207, not by this handbook.
> - **The change-history CI guard it refers to does not exist.** It was part of the same
>   July branch and never merged, so nothing enforces that rule today.
> - **Phases 5 and 6 are not finished**, though the roadmap said so at the time: `KES` is
>   still hardcoded in four modules, and `tenant_for_user()` returns the default tenant
>   for every user, so tenant isolation is a built seam and not a live boundary.
>
> `docs/adr/` and `docs/roadmap/README.md` are maintained and win over this document
