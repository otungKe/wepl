...
| **CV-13** | **Extend the grep-guard family** — add guards for newly-identified dangerous patterns (e.g. float in money code, provider vocab above a port) beyond the legacy-ledger guard | New guards merge-blocking in CI | P-22 |
