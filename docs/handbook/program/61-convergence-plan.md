...
| **CV-23** | **De-couple the ledger from the M-Pesa adapter.** *(Increment 1 ✅ Done 2026-07-11, ADR-0030, issue #159.)* `apps/ledger` no longer imports `apps.mpesa`: the dead `MpesaService` import removed, the Daraja status re-query moved behind a new `PaymentProvider.request_payout_result()` port method, and the unreachable `_on_b2c_success` branch deleted — all behaviour-preserving. A CI guard now fails the build if `apps/ledger` imports `apps.mpesa`. **Increment 2 (open):** rename the Daraja-named `FinancialTransaction` fields (`mpesa_checkout_id/conversation_id/receipt`) to rail-agnostic names (migration-bearing). | ✅ no `apps.mpesa` import in ledger + guard green; ⬜ no Daraja vocabulary in ledger models | **Rule 1 / P-18**; ADR-0030 |
