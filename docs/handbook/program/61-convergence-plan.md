...
> **Written 2026-07-11; not re-verified since.** This handbook predates ADR-0028
> through ADR-0032 and the `FinancialTransaction` decomposition now half-run
> (ADR-0030). Treat any specific claim here as needing a check against
> `docs/adr/` and `docs/roadmap/README.md`, which are maintained.
> See [the handbook README](../README.md) for what is known to have changed.
...
| **CV-23** | **De-couple the ledger from the M-Pesa adapter.** *(Increment 1 ✅ Done 2026-07-11, ADR-0030, issue #159.)* `apps/ledger` no longer imports `apps.mpesa`: the dead `MpesaService` import removed, the Daraja status re-query moved behind a new `PaymentProvider.request_payout_result()` port method, and the unreachable `_on_b2c_success` branch deleted — all behaviour-preserving. A CI guard now fails the build if `apps/ledger` imports `apps.mpesa`. **Increment 2 ✅ Done 2026-09-23:** the Daraja-named `FinancialTransaction` fields were not renamed but removed (`ledger.0021`) — `PaymentIntent` is authoritative for the rail dimension, so a rail-agnostic column on the ledger would have been a second copy of what the intent already holds. A second CI guard fails the build if a rail-named field returns to a ledger model. | ✅ no `apps.mpesa` import in ledger + guard green; ✅ no Daraja vocabulary in ledger models | **Rule 1 / P-18**; ADR-0030 |
