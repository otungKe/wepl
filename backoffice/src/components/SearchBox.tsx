...
const TYPE_LABEL: Record<string, string> = { user: 'User', community: 'Community', verification: 'KYC', journal: 'Journal' }
...
export function SearchBox() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    const t = setTimeout(() => {
      ops.search(term).then((r) => { setResults(r.data.results); setOpen(true) })
        .catch(() => setResults([])).finally(() => setLoading(false))
    }, 220)
    return () => clearTimeout(t)
  }, [q])

  const go = (r: OpsSearchResult) => { setOpen(false); setQ(''); router.push(r.url === '/admin' ? '/' : r.url.replace(/^\/admin/, '')) }
