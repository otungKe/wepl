  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white/50 px-6 py-16 text-center dark:border-slate-700 dark:bg-slate-900/40">
      ...
      <p className="mt-1 max-w-md text-sm text-slate-500">
        ...
        {item?.phase ? ` ${item.phase}` : ' an upcoming phase'}. Navigation, permissions and deep-links are
        already wired — the module&apos;s queues and detail views arrive next.
      </p>
      {slug.length > 1 && <p className="mt-3 font-mono text-xs text-slate-400">/{slug.join('/')}</p>}
    </div>
  )
}
