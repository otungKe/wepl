...
import {
  ...
  LayoutDashboard, Users, Settings2, History, type LucideIcon,
} from 'lucide-react'
import { opsUsers, RESTRICTION_KINDS, type User360, type MoneyActivity } from '@/lib/platform'
...
export default function User360Page() {
  ...
  const activeRestrictions = data.restrictions.filter((r) => r.is_effective)
  const kycOutstanding = (data.verification.resubmission_requested?.length ?? 0) > 0
  const alerts: { label: string; tab: string }[] = [
    ...activeRestrictions.map((r) => ({ label: r.kind_label, tab: 'security' })),
    ...(data.financial.open_holds > 0 ? [{ label: `${data.financial.open_holds} open hold(s)`, tab: 'financial' }] : []),
    ...(kycOutstanding ? [{ label: 'KYC re-submission pending', tab: 'profile' }] : []),
  ]
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-1.5 flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-400">
        ...
        {alerts.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {alerts.map((a, idx) => (
              <button key={idx} onClick={() => setTab(a.tab)}
                className="flex items-center gap-1 rounded-full bg-amber-100 px-2.5 py-0.5 text-[11px] font-semibold text-amber-700 hover:bg-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:hover:bg-amber-500/20">
                <ShieldOff className="h-3 w-3" /> {a.label}
              </button>
            ))}
          </div>
        )}
      ...
      {(err || flash) && (
        <div className="mt-4">
          {err && <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600 dark:bg-red-500/10 dark:text-red-400">{err}</p>}
      ...
      <div className="mt-4 space-y-5">
        ...
        {tab === 'overview' && (
          <>
            <Card title="At a glance">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-1.5 border-t border-slate-100 pt-3 text-sm dark:border-slate-800">
                ...
                {activeRestrictions.length > 0 && (
                  <span className="text-amber-600 dark:text-amber-400">{activeRestrictions.length} active restriction(s)</span>
                )}
                <span className="text-slate-400">{data.sessions.active} active device(s)</span>
              </div>
            </Card>

            <Card title="Recent money activity">
              <ActivityList items={data.recent_activity.slice(0, 5)} />
              {data.recent_activity.length > 5 && (
                <button onClick={() => setTab('financial')}
                  ...
                  View all {data.recent_activity.length} movements →
                </button>
              )}
            </Card>
          </>
        ...
        {tab === 'profile' && (
          <>
            ...
            <Card title="Contact & address">
              {!data.contact.has_kyc ? (
                ...
                <>
                  ...
                  <p className="mt-2 text-[10px] text-slate-400">Phone changes go through the maker-checked flow in Actions.</p>
                </>
              ) : (
            ...
            {canManage && (
              <Card title="Correct display name">
                ...
                <p className="mt-1.5 text-[10px] text-slate-400">Display name only — legal name follows the ID on file.</p>
              </Card>
            )}
          </>
        )}
        ...
        {tab === 'actions' && (
          <>
            {canManage ? (
              <Card title={i.is_active ? 'Deactivate account' : 'Reactivate account'}>
                {i.is_active ? (
                  <>
            ...
            {canManage && (
              <Card title="Change phone number">
                <label className="mb-1 flex items-center gap-1.5 text-[11px] font-semibold text-slate-500"><PhoneCall className="h-3 w-3" /> Maker-checked — needs a second operator</label>
...
function ActivityList({ items }: { items: MoneyActivity[] }) {
  if (items.length === 0) return <p className="text-sm text-slate-400">No money movements on record.</p>
  return <div className="space-y-1">{items.map((a) => <ActivityRow key={a.id} a={a} />)}</div>
}

function ActivityRow({ a }: { a: MoneyActivity }) {
  const out = a.direction === 'PAYOUT'
  return (
    <Link href={`/transactions/${a.id}`}
