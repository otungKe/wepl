...
their own rail). Write side: per-movement requery / re-send / mark-failed /
confirm-paid, routed through ``PaymentOpsService`` (the one door — no button
bypasses the pipeline) and gated behind ``finops.retry`` + a fresh step-up, with
a dual audit trail.
"""
...
class FinopsActionView(OpsAPIView):
    """POST /api/ops/finops/transactions/<ft_id>/action/ {action, reason, receipt}
    — action ∈ {requery, retry_payout, mark_failed, confirm_paid}. Routes through
    PaymentOpsService; every call is step-up-gated and written to the ops audit
    ...
    def post(self, request, ft_id):
        ...
        receipt = (request.data.get("receipt") or "").strip()
        ...
        try:
            if action == "requery":
                ...
                result = PaymentOpsService.confirm_paid(ft, receipt=receipt, actor_label=actor_label)
            else:
        ...
        if action == "confirm_paid":
            metadata["receipt"] = receipt.upper()
        record_action(
