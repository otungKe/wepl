"""Quarantined legacy tests for the Daraja webhook views.

These moved here with the views themselves (ADR-0033): ``STKCallbackView``,
``C2BCallbackView`` and ``B2CResultView`` are ``apps.payments`` endpoints now.
They are still skipped; the live coverage of the same views is in
``tests_mpesa_callbacks.py``.
"""
...
class STKCallbackViewTest(TestCase):
    ...
    @patch("apps.mpesa.tasks.process_stk_sync")
